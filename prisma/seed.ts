...
async function main() {
  ...
  // Create admin user
  const adminPasswordHash = await bcrypt.hash("admin456", 12);
  ...
  console.log(
    "Created staff users:",
    staffUsers.map((s) => s.username).join(", ")
  );

  // Create queue settings for each customer type
  const queueSettings = [
    {
      customerType: CustomerType.instant,
      prefix: "I",
      currentNumber: 0,
      maxNumber: 999,
    ...
    {
      customerType: CustomerType.browser,
      prefix: "B",
      currentNumber: 0,
      maxNumber: 999,
  ...
  const sampleTokens = [
    { customerType: CustomerType.instant, priority: 0 },
    { customerType: CustomerType.instant, priority: 0 },
    { customerType: CustomerType.browser, priority: 0 },
    { customerType: CustomerType.retail, priority: 1 },
    { customerType: CustomerType.instant, priority: 0 },
    { customerType: CustomerType.browser, priority: 0 },
    { customerType: CustomerType.retail, priority: 0 },
  ];
  ...
  for (const tokenData of sampleTokens) {
    ...
    const tokenNumber = `${setting.prefix}${counters_data[
      tokenData.customerType
    ]
      .toString()
      .padStart(3, "0")}`;
