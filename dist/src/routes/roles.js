 ...
 */
router.get("/", auth_1.authenticate, (0, auth_1.authorize)([client_1.UserRole.admin, client_1.UserRole.super_admin]), (0, errorHandler_1.asyncHandler)(async (req, res) => {
 ...
 *         description: Insufficient permissions
 */
router.post("/", auth_1.authenticate, (0, auth_1.authorize)([client_1.UserRole.super_admin]), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const validatedData = createRoleSchema.parse(req.body);
    // For now, we only support default roles
    // Custom role creation would require a Role model in the database
    return res.status(400).json({
        success: false,
        message: "Custom role creation not supported. Only default roles (super_admin, admin, staff) are available.",
    });
}));
/**
 * @swagger
 ...
 */
router.get("/:roleId", auth_1.authenticate, (0, auth_1.authorize)([client_1.UserRole.admin, client_1.UserRole.super_admin]), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (defaultRoles.includes(roleId)) {
        ...
        const roleInfo = {
            id: roleId,
            name: roleId === "super_admin"
                ? "Super Admin"
                : roleId === "admin"
                    ? "Admin"
                    : "Staff",
            description: roleId === "super_admin"
                ? "Full system access"
                : roleId === "admin"
    ...
    return res.status(404).json({
        success: false,
        message: "Role not found. Only default roles (super_admin, admin, staff) are supported.",
    });
}));
/**
 * @swagger
 ...
 *         description: Insufficient permissions
 */
router.put("/:roleId", auth_1.authenticate, (0, auth_1.authorize)([client_1.UserRole.super_admin]), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { roleId } = req.params;
    ...
    return res.status(400).json({
        success: false,
        message: "Role modification not supported. Only default roles (super_admin, admin, staff) are available.",
    });
}));
/**
 * @swagger
 ...
 *         description: Insufficient permissions
 */
router.delete("/:roleId", auth_1.authenticate, (0, auth_1.authorize)([client_1.UserRole.super_admin]), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { roleId } = req.params;
    ...
    return res.status(400).json({
        success: false,
        message: "Role deletion not supported. Only default roles (super_admin, admin, staff) are available.",
    });
}));
