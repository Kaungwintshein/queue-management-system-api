...
async function main() {
    ...
    const hashedPasswords = {
        admin: await bcryptjs_1.default.hash("admin123", 10),
        manager: await bcryptjs_1.default.hash("admin456", 10),
        staff1: await bcryptjs_1.default.hash("staff123", 10),
        staff2: await bcryptjs_1.default.hash("staff456", 10),
        superadmin: await bcryptjs_1.default.hash("superadmin123", 10),
    };
    const users = [
        {
    ...
    for (const counterData of counters) {
        ...
        console.log(`✅ Created test counter: ${counter.name} (${counter.isActive ? "Active" : "Inactive"})`);
    }
    ...
    for (const settingData of queueSettings) {
        ...
        console.log(`✅ Created queue setting: ${setting.customerType} (${setting.prefix})`);
    }
    // Create test tokens
    const tokens = [
        {
    ...
    for (const sessionData of sessions) {
        ...
        console.log(`✅ Created test session: ${session.id} (${session.endedAt ? "Ended" : "Active"})`);
    }
    ...
    console.log(`- Queue Settings: ${queueSettings.length} (instant, browser, retail)`);
